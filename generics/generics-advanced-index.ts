...
// Narrowing using the 'in' operator
function getRuntime(media: Movie | TVShow): number {
  // Check if the 'duration' property exists in the media object
  // If it does, it's a Movie; otherwise, it's a TVShow
  if ("duration" in media) {
    return media.duration; // Movie
  } else {
    return media.episodes * media.episodeDuration; // TVShow
  }
}
